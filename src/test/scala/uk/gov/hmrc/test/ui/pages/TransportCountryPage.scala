...
object TransportCountryPage extends BasePage {
  ...
  def title: String                 =
    s"Select the country where the ${store.getOrElse(TransportLeavingBorderValue, "default value").toLowerCase} is registered"
  ...
  def performActionsAndStore(country: String*): Unit = {
    fillAutoComplete(findElementById("transport-country"), country.head)
    store += (TransportCountryId -> "Country of registration for the transport leaving the UK border", TransportCountryValue -> country)
  }
