...
object ProcedureChoicePage extends BasePage {
  ...
  def backButtonHref: String = if (isClearance) {
    ...
    maybeDetail(NoAdditionalPartiesInvolved) match {
      case Some(_) =>
        maybeDetail(NoAdditionalPartiesInvolved).fold(OtherPartiesInvolvedPage.path)(_ =>
          OtherPartiesInvolvedListPage.path
        )
      case _ =>
        maybeDetail(NoAdditionalPartiesInvolved).fold(OtherPartiesInvolvedListPage.path)(_ =>
          OtherPartiesInvolvedPage.path
        )
    }
